...
function updatevalue(vendorid,codeid,val){
	if(codeid=='2'){
		var data = '_csrf='+_csrf+'&vendorid='+vendorid+'&codeid='+codeid+'&val='+val;
	}else{
		var data = '_csrf='+_csrf+'&vendorid='+vendorid+'&codeid='+codeid;
	}
	$.ajax({
 		type: 'POST',
 		url: '/updatevalue',
...
function finalOrder(){
	...
	$.ajax({
		...
		url: "/checkorder",
		dataType: "json",
		data: '_csrf='+_csrf+'&address='+address+'&payment='+payment,
		cache: false,
		success:function(response){
