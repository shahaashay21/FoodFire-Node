...
exports.restaurants = function (req, res) {
    try {
        ...
        let likes = " (select vendorunkid, COUNT(*) as likes from ratings where code_id=1 group by vendorunkid) as l ON v.vendorunkid = l.vendorunkid";
        let rating = " (select vendorunkid, ROUND(AVG(rate), 1) as ratings from ratings where code_id=2 group by vendorunkid) as r ON v.vendorunkid = r.vendorunkid";
        let favorite = " (select vendorunkid, COUNT(*) as favorites from ratings where code_id=3 group by vendorunkid) as f ON v.vendorunkid = f.vendorunkid";
        let visit = " (select vendorunkid, COUNT(*) as visits from ratings where code_id=4 group by vendorunkid) as vi ON v.vendorunkid = vi.vendorunkid";
        let votes = " (select vendorunkid, COUNT(*) as votes from ratings where code_id=5 group by vendorunkid) as vo ON v.vendorunkid = vo.vendorunkid";
